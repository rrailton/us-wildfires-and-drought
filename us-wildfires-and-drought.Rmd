...
```{r, libraries, message = FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, loading-wildfires, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, na-count, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
glimpse(fires)
head(fires, n = 20L)
...
```{r, transform, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, create-subsets, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# create CA df
...
```{r, import-drought-us, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, import-drought-ca, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, combine-spei, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, summarise-data, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, descriptive-stats, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
attach(fires_new)
...
```{r, histograms, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# Histograms
...
```{r, boxplots, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```
## Visualizations
```{r, visualizations, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
glimpse(fires_new)
...
```
```{r, fire-causes, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# Fires Causes
...
```{r, correlation-matrix-attributes, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# US fires
...
```{r, fire-size-spei-linear-regression, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# CA FIRE_SIZE & SPEI
...
```{r, fire-freq-spei-linear-regression, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# CA FIRE_FREQ & SPEI
...
```{r, fire-area-spei-linear-regression, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# CA FIRE_AREA & SPEI
...
```{r, fires-large-spei-linear-regression, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
...
```{r, multiple-linear-regression, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
# merge fa,fs,lf
...
```{r, fire-size-prediction, tidy=TRUE, tidy.opts=list(width.cutoff=80)}
